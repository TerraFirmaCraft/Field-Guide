...
class Loader:
    ...
    def load_config_defaults(self) -> Any:
        """
        Loads the default values of every $(cfg:) macro the field guide uses, as generated by TFC's data generators.
        Versions of TFC older than that generator don't have the file, and don't use the macro either, so an absent
        file is not an error.
        """
        path = util.path_join(self.tfc_dir, 'src/generated/resources/assets/tfc/field_guide/config_defaults.json')
        try:
            with open(path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except OSError:
            util.LOG.info('No config defaults at %s, assuming this version does not use them' % path)
            return {}
